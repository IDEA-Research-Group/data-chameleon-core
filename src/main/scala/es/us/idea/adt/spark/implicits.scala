...
import es.us.idea.adt.dsl.NamedFieldContainer
...
object implicits {
  ...
  implicit class DFADT(df: DataFrame) {
    def adt(dt: NamedFieldContainer*): DataFrame = {
      transformDataFrame(df, dt.map(t => t.build() match {
        case nf: NamedField => nf
        case _ => throw new Exception("adt method must receive named fields")
      }): _*)
    }
  }

  private def transformDataFrame(df: DataFrame, dt: NamedField*): DataFrame = {
